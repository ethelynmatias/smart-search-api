...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="container">
            ...
            @if ($showPayload)
                {{-- One flow, read as a progress trail: oldest step at the top,
                     newest at the bottom, each with its payload attached. --}}
                @php($previous = null)

                <ol class="timeline">
                    @forelse ($logs as $log)
                        @php($gap = $previous?->diffInSeconds($log->created_at))

                        <li class="timeline-step">
                            <div class="timeline-rail">
                                <span class="timeline-dot {{ $log->type->value }}"></span>
                                <span class="timeline-index">{{ $logs->firstItem() + $loop->index }}</span>
                            </div>

                            <div class="timeline-card">
                                <div class="timeline-head">
                                    <span class="badge {{ $log->type->value }}">{{ $log->type->value }}</span>
                                    <span class="timeline-title">{{ $log->message ?? 'Untitled entry' }}</span>
                                    <time class="timeline-time nowrap">{{ $log->created_at->format('Y-m-d H:i:s') }}</time>
                                </div>
                                ...
                                <div class="timeline-meta">
                                    ...
                                    {{-- How long the flow sat between steps is the
                                         thing a trail is usually being read for. --}}
                                    @if ($gap !== null)
